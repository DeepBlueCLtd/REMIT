...
- **Tech stack:** HTML/CSS/JS app built with **Vite**; an **H3 hexagonal grid**
  ...
  maintainer-approved** (ADR-0014); kernel modules stay dependency-light (only `h3-js`,
...
## Data model — LinkML is the source of truth (DEC-57)

The serialisable object core is defined **once**, in LinkML, split into discrete
modules under `schema/` — `common` (shared value objects + the enum vocabulary),
`requirement`, `world`, `force`, `entities`, `plan`, `records` — stitched by the thin
entry schema **`schema/remit.yaml`** (it imports them all; generators run against it).
Edit the module files; never edit the entry's class list (it has none) or the
generated artefacts. Everything downstream is **generated, never hand-written**:

| Artefact | Path | Generated by |
|----------|------|--------------|
| JSON Schema | `schema/gen/remit.schema.json` | `gen-json-schema` |
| TypeScript types | `schema/gen/remit.ts` | `gen-typescript` |
| Human reference (HTML) | `site/data-model/` | `schema/build-reference.py` |

Regenerate everything with **`bash schema/generate.sh`** (it bootstraps a LinkML
venv — the distro `pip` can't install LinkML; see `bugs.md`). Pydantic is omitted
until a Python consumer exists (one line to add). The generated files in
`schema/gen/` and `site/data-model/` are **outputs** — do not hand-edit them; change
the schema and re-run.

**The rules (adapted from the maintainer's constitution — ADR-0012):**

- **One source.** LinkML defines the data structures; TS / JSON Schema (/ later
  Pydantic) are derived. **Never hand-author a type the schema could generate.**
  Writing-time habit: *before declaring a non-trivial shared shape in `app/js`,
  import the generated type instead of re-listing its fields* — re-listing silently
  drops fields the source later grows.
- **UI-only, single-class discrete types are exempt.** Display/view-only shapes used
  in one place (the Sync-Matrix catalogue, coincidence rows, render closures,
  `main.js` UI state) stay hand-written. The schema models the *data* core only.
- **Cross-boundary subset types** (when a boundary ever appears — none in v1's single
  browser process) must be structural (`Pick`/`Omit`/`Partial`/a derived validator),
  never re-listed by hand.
- **Scope:** LinkML models *data, not behaviour* — function-valued fields
  (`Channel.values`, `MovementModel.cost_speed`, `Aspect.value`) and endpoints stay in
  the seam contract.

The schema is **reconciled against the walking skeleton's real object shapes** so
schema ≡ code; classes the skeleton doesn't build yet are flagged in-schema. The
prose `docs/remit-data-model.md` is the spine the schema was authored from.

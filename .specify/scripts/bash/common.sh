...
get_current_branch() {
    ...
    # >>> REMIT addition (capability #7): tier 2 — .specify/.active-feature.
    # A per-worktree, gitignored override taking precedence over the git branch,
    # for cloud sessions whose forced branch (claude/<topic>-<random>) has no
    # NNN- token. Logic lives in active-feature.sh; re-apply this hunk if a future
    # `specify init` overwrites common.sh. See decisions.md (ADR-0004).
    if [[ -f "$repo_root/.specify/scripts/bash/active-feature.sh" ]]; then
        source "$repo_root/.specify/scripts/bash/active-feature.sh"
        local _af
        _af=$(tmpl_read_active_feature "$repo_root")
        if [[ -n "$_af" ]]; then
            echo "$_af"
            return
        fi
...
get_feature_paths() {
    ...
    # >>> REMIT addition (capability #7): recovery hint when nothing resolved.
    # When feature_dir points at a non-existent spec, list the available specs and
    # the recovery options on stderr instead of emitting paths that fail later with
    # no guidance. Callers (setup-plan/tasks, check-prerequisites) already treat a
    # non-zero return as an error. Re-apply this hunk if `specify init` overwrites
    # common.sh. See decisions.md (ADR-0004).
    if [[ ! -d "$feature_dir" ]]; then
        if [[ -f "$repo_root/.specify/scripts/bash/active-feature.sh" ]]; then
            source "$repo_root/.specify/scripts/bash/active-feature.sh"
            tmpl_no_feature_hint "$repo_root"
        fi
        return 1
    fi
    # <<< REMIT addition end

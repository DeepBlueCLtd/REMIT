...
| Map render (003) | **MapLibre GL** + **deck.gl** `H3HexagonLayer`/`PathLayer`/`IconLayer` via `MapboxOverlay`; keyless **Carto Positron (light)** raster basemap behind a **load-guard** (tiles blocked in cloud → degrades to a flat field; real tiles render in-browser); **hex overlay toggleable** via a top-right MapLibre control (`data-hexes` on/off); `app/js/views/map.js` (replaces Canvas `render.js`) |

...
| Map render (003) | **MapLibre GL** + **deck.gl** `H3HexagonLayer`/`PathLayer`/`IconLayer` via `MapboxOverlay`; keyless **Carto dark-matter** raster basemap behind a dark-field **load-guard** (tiles blocked in cloud → degrades to the flat dark field; real tiles render in-browser); **hex overlay toggleable** via a top-right MapLibre control (`data-hexes` on/off); `app/js/views/map.js` (replaces Canvas `render.js`) |

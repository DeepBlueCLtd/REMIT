...
| Deps (003) | `h3-js ^4.4`, `maplibre-gl ^5.24`, `@deck.gl/core|layers|geo-layers|mapbox ^9.3` (runtime); `vite ^8` (dev). Minimised + approval-gated (ADR-0010) |
| Hex grid (003) | H3 **res 9** (~300 m cells, ~1.2k over ~14×9 km), anchored to a real lat/lon (Solway Firth head ≈ 54.96°N, 3.10°W). Stable sorted-id ↔ H3 string; bearing-sorted neighbours; `app/js/kernel/hexgrid.js`. Hierarchy latent (ADR-0012) |
...
| Map render (003) | **MapLibre GL** + **deck.gl** `H3HexagonLayer`/`PathLayer`/`IconLayer` via `MapboxOverlay`; keyless **Carto dark-matter** basemap; `app/js/views/map.js` (replaces Canvas `render.js`) |
| Hex scenario (003) | "Solway crossing" — river delta with **multiple tidal fords** (waths); base inland · OP(s) over the channels · RV on the far bank; spec + evidence in `specs/003-hex-grid-routing/` |
